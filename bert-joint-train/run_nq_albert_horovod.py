...
def model_fn_builder(bert_config, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            start_loss_hook = tf.train.LoggingTensorHook({"start_loss": start_loss}, every_n_iter=100)
            end_loss_hook = tf.train.LoggingTensorHook({"end_loss": end_loss}, every_n_iter=100)
            answer_type_loss = tf.train.LoggingTensorHook({"answer_type_loss": answer_type_loss}, every_n_iter=100)
            ...
            output_spec = contrib_tpu.TPUEstimatorSpec(
                ...
                training_hooks=[start_loss_hook, end_loss_hook, answer_type_loss])
...
def main(_):
    hvd.init()
    FLAGS.output_dir = FLAGS.output_dir if hvd.rank() == 0 else os.path.join(FLAGS.output_dir,
                                                                             'trash/' + str(hvd.rank()))
    ...
    if hvd.rank() == 0:
        FLAGS.save_checkpoints_steps = FLAGS.save_checkpoints_steps
    else:
    ...
    tf.logging.info(config.gpu_options.visible_device_list)
    ...
    if FLAGS.do_train:
        ...
        num_train_steps = int(num_train_features / (FLAGS.train_batch_size * hvd.size()) *
